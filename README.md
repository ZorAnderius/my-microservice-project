...
- [Моніторин з Grafana](#моніторин-з-grafana)
...
|       |
|       ├── monitoring/               # Модуль для Grafana
│       │   ├── monitoring.tf         # Створення Grafana  
│       │   ├── values.yaml           # Конфігурація Grafana (ресурси, доступ) 
│       │   ├── variables.tf          # Змінні (ресурси, креденшели, values)
│       │   └── outputs.tf            # вихідні дані від Grafana
│       │
...
В секції створення інфраструктури видно, що після успішного виконання скрипта **init-all.sh** виводиться посилання на сторінку Grafana для моніторнигу стану подів в Kubernetes кластері  (та інших параметрів). Також там наведено логін та пароль для даного сервісу.
Нижче наведено результат моніторингу поду з django-застосунком (CPU, memory etc.)

![Моніторинг Django-app  з допомогою Grafana](assets/grafana.jpg)
